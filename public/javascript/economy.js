...
var CInventory = Class.create( {
		...
		for( var sCategoryName in this.tags )
		{
			...
			for( var index in rgCategoryTags )
			{
				...
				if ( ++cTagsDisplayed == 5 && cTagsTotal > 7 )
				{
					...
					elExpandTags.update( '+ Показать еще' );
	...
	BuildUnknownItemElement: function( appid, contextid, itemid )
	{
		...
		elItem.rgItem = { unknown: true, id: itemid, appid: this.appid, contextid: this.contextid, element: elItem, name: 'Неизвестный предмет ' + itemid, descriptions: [], fraudwarnings: [ 'Не удалось получить информацию об этом предмете.' ] };
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( 'Все предметы #{appname}');
	...
	OnInventoryLoadFailed: function( transport, appid, contextid )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.busy )
		{
			if ( !elFailedInventory )
			{
				...
				alert( 'Этот инвентарь в данный момент недоступен. Повторите попытку позже.' );
			}
		}
...
var g_rgEconomyDisplay = {"440":{"howtoget":"\u0412\u044b \u043c\u043e\u0436\u0435\u0442\u0435 \u043f\u043e\u043b\u0443\u0447\u0438\u0442\u044c \u0438\u0445 \u043f\u0443\u0442\u0435\u043c \u0441\u043b\u0443\u0447\u0430\u0439\u043d\u043e\u0433\u043e \u0432\u044b\u043f\u0430\u0434\u0435\u043d\u0438\u044f \u0432 \u0438\u0433\u0440\u0435, \u043a\u0443\u043f\u0438\u0432 \u0432\u043e \u0432\u043d\u0443\u0442\u0440\u0438\u0438\u0433\u0440\u043e\u0432\u043e\u043c \u043c\u0430\u0433\u0430\u0437\u0438\u043d\u0435 \u041c\u0430\u043d\u043d \u041a\u043e \u0438\u043b\u0438 \u0432\u044b\u043c\u0435\u043d\u044f\u0442\u044c \u0438\u0445 \u0443 \u0434\u0440\u0443\u0437\u0435\u0439."},"620":{"howtoget":"\u0412\u044b \u043c\u043e\u0436\u0435\u0442\u0435 \u043f\u0440\u0438\u043e\u0431\u0440\u0435\u0441\u0442\u0438 \u0438\u0445 \u0432\u043e \u0432\u043d\u0443\u0442\u0440\u0438\u0438\u0433\u0440\u043e\u0432\u043e\u043c \u043c\u0430\u0433\u0430\u0437\u0438\u043d\u0435 Portal 2 \u0438\u043b\u0438 \u0432\u044b\u043c\u0435\u043d\u044f\u0442\u044c \u0438\u0445 \u0443 \u0434\u0440\u0443\u0437\u0435\u0439."},"753":{"1":{"howtoget":"\u0412\u044b \u043c\u043e\u0436\u0435\u0442\u0435 \u043f\u043e\u043b\u0443\u0447\u0438\u0442\u044c \u0434\u043e\u043f\u043e\u043b\u043d\u0438\u0442\u0435\u043b\u044c\u043d\u044b\u0435 \u043a\u043e\u043f\u0438\u0438 \u0438\u0433\u0440 \u0432\u043e \u0432\u0440\u0435\u043c\u044f \u0441\u043f\u0435\u0446\u0438\u0430\u043b\u044c\u043d\u044b\u0445 \u043f\u0440\u0435\u0434\u043b\u043e\u0436\u0435\u043d\u0438\u0439 \u0438\u043b\u0438 \u043f\u0440\u0438 \u043f\u043e\u043a\u0443\u043f\u043a\u0435 \u0438\u0433\u0440\u044b \u0432 \u043c\u0430\u0433\u0430\u0437\u0438\u043d\u0435 Steam, \u0432\u044b\u0431\u0440\u0430\u0432 \u043f\u0430\u0440\u0430\u043c\u0435\u0442\u0440 \u00ab\u041a\u0443\u043f\u0438\u0442\u044c \u0432 \u043f\u043e\u0434\u0430\u0440\u043e\u043a\u00bb \u0432\u043e \u0432\u0440\u0435\u043c\u044f \u043f\u043e\u043a\u0443\u043f\u043a\u0438."}},"99900":{"logo":"https:\/\/steamcommunity-a.akamaihd.net\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/steamcommunity-a.akamaihd.net\/public\/images\/economy\/applogos\/99900.png"}};
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> из #{contextname}');
...
function BuildHover( prefix, item, owner )
{
	...
	if ( elFraudWarnings )
	{
		...
		if ( item.fraudwarnings || ( g_bIsInventoryPage && item.is_currency ) )
		{
			...
			{
				var warning = $J( '<div/>' );
				warning.text( 'Это значение видите только вы.' );
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s шт.'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elMarketLink.update( 'Найти на Торговой площадке' );
		...
		elPriceInfoContent.update( '<img src="https://steamcommunity-a.akamaihd.net/public/images/login/throbber.gif" alt="Обработка…">' );
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', 'Продать' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'Вы сможете продавать вещи на Торговой площадке сообщества, только когда <a href="http://store.steampowered.com/steamaccount/addfunds" target="_top">пополните баланс кошелька Steam</a> или сделаете покупку в магазине Steam и предоставите платежный адрес.',
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( 'Выставление предмета на продажу' );
	...
	OnAccept: function( event ) {
		...
		{
			this.DisplayError( 'Для продажи предмета примите условия соглашения подписчика Steam.' );
			return;
		}
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( 'Введите корректное значение.' );
			return;
		}
		...
		if ( buyerPrice > g_rgWalletInfo['wallet_trade_max_balance'] || price > g_rgWalletInfo['wallet_trade_max_balance'] )
		{
			...
			var strError = ' Введенная цена превышает максимальную цену в %1$s.'
		...
		{
			$('market_sell_currency_input').style.borderColor = 'red';
			this.DisplayError( 'Необходимо указать действительную цену.' );
			return;
		}
		...
		$('market_sell_dialog_title').update( 'Подтвердите цену продажи' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = 'Чтобы выставить этот предмет на Торговую площадку, подтвердите это действие через мобильное приложение Steam.' +
							'<br><br>' +
							'Если вы не видите раздел подтверждений в главном меню, то убедитесь, что у вас установлена последняя версия приложения.';

				}
				else
				{
					strText = 'Для продажи этого предмета на Торговой площадке требуется ваше подтверждение. Все необходимые инструкции были высланы на ваш адрес электронной почты, заканчивающийся на %s.'.replace( /%s/, transport.responseJSON.email_domain );
				}

				ShowAlertDialog(
						'Необходимо дополнительное подтверждение',
						strText
				);
			}
			else
		...
		{
			this.DisplayError( 'Ошибка при выставлении предмета на продажу. Обновите страницу и повторите попытку.' );
		}
	},
...
function SelectItemDialogOnSelect()
{
	...
	{
		alert( 'Произошла ошибка во время сохранения вашего выбора. Попробуйте еще раз позже.' );
	}
}
...
function CreatePriceHistoryGraph( line1, numYAxisTicks, strFormatPrefix, strFormatSuffix )
{
	var plot = $J.jqplot('pricehistory', [line1], {
		title:{text: 'Медиана цен', textAlign: 'left' },
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( 'Если пользователь %s пытался вас обмануть, сообщите об этом службе поддержки Steam.<br><br>Пожалуйста, вкратце опишите ситуацию:'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( 'Сообщить о мошенничестве', $Dialog, 'Сообщить о мошенничестве').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( 'Сообщить о мошенничестве', 'Требуется описание.' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( 'Спасибо!', 'Жалоба отправлена. Служба поддержки Steam рассмотрит её.' );
		}).fail( function() {
			ShowAlertDialog( 'Сообщить о мошенничестве', 'При попытке пожаловаться произошла ошибка. Пожалуйста, попробуйте позже.' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">Что такое удержание обменов или продаж?</div>' );
	description.append( '<div class="escrow_explanation_a">Удержание — это небольшой промежуток времени, в течение которого Steam хранит предметы до завершения обмена или выставления лота на Торговой площадке.</div>' );

	description.append( '<div class="escrow_explanation_q">Удержание предметов помогает защитить ваш инвентарь</div>' );
	description.append( '<div class="escrow_explanation_a">Аккаунт Steam является желанной добычей взломщиков, особенно если на нём есть ценные предметы. Если вы не защитили свой аккаунт аутентификатором Steam на мобильном устройстве, то удержание предметов даст вам время на то, чтобы заметить взлом аккаунта и предотвратить кражу.</div>' );

	description.append( '<div class="escrow_explanation_q">Задержка для обнаружения и предотвращения кражи предметов</div>' );
	description.append( '<div class="escrow_explanation_a">Если аккаунт, с которого совершают обмен или продажу предметов, не был защищён мобильным аутентификатором Steam на протяжении предыдущих 7 дней: <br><span style="color: #5aa9d6">Обмен:</span> передача предметов будет отсрочена на 15 сут. <br><span style="color: #5aa9d6">Торговая площадка:</span> выставление предметов на продажу будет отсрочено на 15 сут. <br><br>Таким образом пользователи успеют отменить любую сделку, которую они не совершали сами. <br><br>Если отменить все незавершённые сделки, обмен будет временно заблокирован: это помешает передать предметы без вашего участия. Блокировка не применяется, если вы сняли предметы с продажи на Торговой площадке.</div>' );

	description.append( '<div class="escrow_explanation_q">Как избежать удержания предметов?</div>' );
	description.append( '<div class="escrow_explanation_a">Вы можете повысить уровень безопасности вашего аккаунта, воспользовавшись <a href="http://store.steampowered.com/mobile/" target="_blank">мобильным аутентификатором Steam Guard</a> для устройств с iOS, Android и Windows. Так вы сможете быстро и безопасно совершать сделки со своего аккаунта, избежав удержания предметов.</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">Прочитайте публикацию в блоге Steam о безопасности аккаунтов</a>, чтобы узнать, почему мы ввели удержание обменов и торговли.</div>' );

	if ( bAddSteamGuardLink )
	{
		var dialog = ShowConfirmDialog(
			'Удержание обменов и продаж',
			description,
			'Подробнее о мобильном аутентификаторе Steam',
			'Закрыть'
		);
